...
3. Required fields: `name`, `tags`, `figma`, `file`, `about`, `desc`. Optional: `variants[]`, `ecosystem`, `brandUrl`. **`about`/`desc` are mandatory, not nice-to-have** — every SEO page renders them (`about` in the body copy, `desc` in `<meta description>`); a logo without them ships with a thin/empty page. `test-data.js --pre` does NOT currently gate on this (only `name`/`tags`/`figma`/`file` fail the build) — that's a test-data.js gap, not license to skip the fields. There's an existing backlog of 121 logos missing `about` (tracked in agent memory, prioritized by download-stats.json) — don't add to it.
...
- **Движок — `scripts/lib/typograf.js`, дубликат `figna-plagins/Trace Typograf/code.js`** (Figma-плагин «Trace Typograf», см. его README). Дубликат, не общий модуль: плагин выполняется в песочнице Figma без `require()`/файловой системы, поэтому вынести правила в единственный источник и подключать из обоих мест нельзя. **Меняешь правило типографики — правь оба файла.**
...
**Production host is Vercel again** (as of 2026-07-17), not GitHub Pages. The GitHub Pages migration documented below (2026-07-05, `sixxset5-star` account, `.github/workflows/deploy.yml`) was reversed — the user explicitly asked to deploy to Vercel instead. **Vercel account changed again on 2026-08-08:** project `trace-logos` now lives under account **`arturablyazov5-5100`** (team/scope `arturablyazov5-5100s-projects`, org id `team_EcDiXc050vJYrmzk2c72d3Up`), NOT the intermediate `sixxset5-star` account (org `team_H5VCeyP9smB6SxWJdPtpD2u1`, now stale) and NOT the original `mansurov` account (org `team_5qtHZtJBmfxpP7GJ9GasLi8j`, retired earlier). `.vercel/project.json` in the repo root points at the new org/project — don't "fix" it back to either older id. `trace-logos.ru` was removed from the `sixxset5-star` project and re-added to the new one; DNS itself didn't change (still points at Vercel's edge). Verify with `curl -sI https://trace-logos.ru` (look for `server: Vercel`) and `vercel whoami` (should show `arturablyazov5-5100`, not `sixxset5-star` or `mansurov` — if it shows something else, the local CLI session is stale and needs `vercel login` again, or pass `--token=...`/`--scope=arturablyazov5-5100s-projects` explicitly). Deploy manually with `npm run deploy` (= `vercel --prod --yes --archive=tgz --scope=arturablyazov5-5100s-projects && node scripts/indexnow-ping.js`) — the repo has 15,900+ files, over Vercel's 15,000-file plain-upload limit, so `--archive=tgz` is required, and `--scope` pins the deploy to the current account regardless of which account the local CLI session happens to be logged into. It's unclear whether the `sixxset5-star/trace-logos` GitHub Pages path (`.github/workflows/deploy.yml`, `gh run list --workflow=deploy.yml`) is still wired up as a fallback or fully retired — check with the user before relying on either path.
