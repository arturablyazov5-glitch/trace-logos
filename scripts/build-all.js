...
const STEPS = [
  ...
  { file: 'test-yo-sync.js',               label: 'Тест: словарь ёфикатора в плагине совпадает с resources/yo.js' },
  ...
  { file: 'build-ecosystem-nav.js',        label: 'Синхронизация блока ECOSYSTEMS в js/data.js с logos/ecosystems.json — до test-js.js/минификации' },
  { file: 'test-ecosystem-sync.js',        label: 'Тест: блок ECOSYSTEMS в js/data.js совпадает с logos/ecosystems.json, иконки существуют' },
  ...
  { file: 'build-js-minify.js',            label: 'JS-минификация (js/*.min.js, components/*.min.js) — источники валидны, дальше страницы ссылаются на .min.js' },
  { file: 'build-js-bundles.js',           label: 'JS-бандлы точек входа (граф импортов → один .min.js) — строго после минификации, перезаписывает .min.js входных модулей' },
  ...
  { file: 'build-credits.js',              label: 'Стена благодарностей (credits/) — из credits.json, после logos.json' },
  ...
  { file: 'build-webp-previews.js',        label: 'WebP-превью для PNG/SVG-логотипов (инкрементально)' },
  ...
  { file: 'build-emoji-category-json.js',  label: 'GEO-экспорт для LLM (emoji-index.json, emoji/<slug>.json)' },
  { file: 'build-og-images.js',            label: 'OG-превью логотипов (assets/og/<slug>.png)' },
  { file: 'build-og-extensions.js',        label: 'OG-превью лендингов расширений (assets/og/tools-<slug>.png)' },
  ...
  { file: 'build-figma-pdfjs.js',           label: 'pdf.js, запечённый в ui.html плагина PDF → Figma' },
  { file: 'build-figma-promo.js',          label: 'Промо-блок Trace Logo\'s в ui.html остальных Figma-плагинов — тоже со счётчиками' },
  ...
  { file: 'build-home-categories.js',      label: 'Блок «Категории логотипов» на главной (из logos/manifest.json)' },
  ...
  { file: 'test-dead-code.js',             label: 'js/css/components/templates-partials/scripts-lib без единой ссылки на них — мёртвый код' },
  { file: 'build-redirects.js',            label: '301-редиректы для перенесённых логотипов (vercel.json ← logos/url-aliases.json) — после test-links.js, проверяет destination на реальных страницах' },
  { file: 'build-redirect-pages.js',       label: 'Физические meta-refresh редирект-страницы (для GitHub Pages, у которого нет vercel.json redirects) — из тех же источников, что build-redirects.js; после cleanup-orphaned-pages.js, который иначе снёс бы их как сирот' },
