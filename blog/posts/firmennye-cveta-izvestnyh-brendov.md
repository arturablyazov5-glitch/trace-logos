---
title: Фирменные цвета известных брендов — HEX‑коды и где брать официальные
title_en: Famous Brand Colors — HEX Codes and Where to Find Official Ones
description: HEX‑коды фирменных цветов Сбера, Яндекса, VK, Т‑Банка, Google и других брендов. Почему у «жёлтого Яндекса» есть точный код и чем грозит цвет «на глаз».
description_en: Official HEX codes for Sber, Yandex, VK, Google and other brands — and why using a color "by eye" is a mistake.
date: 2026-06-10
slug: firmennye-cveta-izvestnyh-brendov
tags: Цвета, Бренды, Дизайн
...
## Цвета российских брендов

Начнём с готовых кодов — дальше объясним, откуда они берутся и почему им можно доверять. Вот ядро палитр брендов, которые вы видите каждый день:

| Бренд | Цвет | HEX |
| --- | --- | --- |
| [Сбер](../../logos/bank/sber/) | зелёный | #21A038 |
| [Яндекс](../../logos/search/yandex/) | жёлтый/красный | #FFCC00 / #FC3F1D |
| [VK](../../logos/social/vk/) | синий | #0077FF |
| [Т‑Банк](../../logos/bank/tbank/) | жёлтый | #FFDD2D |
...
Источников по сути три, и они выстраиваются по убыванию надёжности:
...
Охраняемый статус цвета не запрещает касаться его вовсе. Показывать бренд его же цветом — нормально: кнопка «Войти через [VK](../../logos/social/vk/)» в синем #0077FF, список банков с их фирменными иконками, обзор сервисов с их палитрами. Это то же номинативное (называющее) использование, что и с логотипами: вы указываете на бренд, оставаясь собой. Запрещено другое — строить **свой** фирменный стиль на чужом охраняемом цвете в той же товарной категории. Жёлтый банк, зелёный банк и красный оператор уже заняты, и заход на эту территорию заканчивается иском.

Раз уж речь зашла о том, что цвет бывает «занят», стоит разобраться, как вообще цвет можно закрепить за собой юридически.

## Можно ли «запатентовать» цвет
...
Есть ещё одна причина, по которой один HEX перестал закрывать все случаи, — тёмные интерфейсы. Цвет, выверенный для белого фона, на чёрном ведёт себя иначе: насыщенный синий «вибрирует», тёмно‑зелёный проваливается, жёлтый начинает кричать. Поэтому зрелые дизайн‑системы держат **две версии палитры**: для светлой темы — базовые коды из брендбука, для тёмной — скорректированные, обычно чуть светлее и менее насыщенные.

Для тех, кто использует чужие фирменные цвета в своём интерфейсе, вывод прямой: проверяйте кнопки «Войти через…» и иконки банков в обеих темах. Официальный цвет на неродном фоне может требовать подложки — именно поэтому гайдлайны платёжных систем так подробно расписывают, на каком фоне какую версию значка ставить.

## Частые вопросы

**Почему цвет с официального сайта не совпал с брендбуком?** Три причины: на сайте ещё не доехал свежий редизайн; вы сняли цвет пипеткой с JPG (искажения); дизайнеры сознательно используют «интерфейсную» версию цвета, отличную от печатной. Приоритет доверия: свежий брендбук → SVG‑логотип → сайт.

**Как узнать цвет бренда без публичных гайдлайнов?** SVG‑логотип — самый надёжный источник: цвет записан в коде файла. Скачайте вектор и посмотрите `fill` — как именно, показано в статье [как узнать цвет логотипа](../kak-uznat-cvet-logotipa/).

**Сколько цветов в палитре бренда?** Ядро — 1‑2 фирменных, плюс функциональные: 2‑4 нейтральных (текст, фоны) и семантические (успех/ошибка/предупреждение). Палитры на десяток «фирменных» цветов встречаются у экосистем, где каждому сервису нужен свой акцент.
...
Let's start with ready codes — then explain where they come from and why you can trust them. Here's the core of palettes you see every day:

| Brand | Color | HEX |
| --- | --- | --- |
| [Sber](../../logos/bank/sber/) | green | #21A038 |
| [Yandex](../../logos/search/yandex/) | yellow/red | #FFCC00 / #FC3F1D |
| [VK](../../logos/social/vk/) | blue | #0077FF |
| [T-Bank](../../logos/bank/tbank/) | yellow | #FFDD2D |
| [Ozon](../../logos/market/ozon/) | blue | #005BFF |
| [Wildberries](../../logos/market/wildberries/) | purple | #CB11AB |
| [Avito](../../logos/market/avito/) | green/blue | #97CF26 / #00AAFF |
| [MTS](../../logos/bank/mts/) | red | #E30611 |
...
| [WhatsApp](../../logos/social/whatsapp/) | green | #25D366 |
| [Spotify](../../logos/media/spotify/) | green | #1DB954 |
| [Netflix](../../logos/media/netflix/) | red | #E50914 |

:::warning Shades change with redesigns
These codes are a snapshot for today, and they don't stay constant. Brands update palettes: "[VK](../../logos/social/vk/) blue" before and after the 2021 rebrand are different colors, and [Meta](../../logos/social/meta/)'s blue was refined too. Doing something official — check the brand's current guideline: a five-year-old article will mislead you.
:::

## Why "by eye" is expensive

With the codes in front of you, the question arises: does precision really matter if the shade almost matched? It does, and here are three reasons, each of which turns "almost" into a real loss.

The first is **recognition**. It rests exactly on the accuracy of the hit: the instant "oh, that's [Sber](../../logos/bank/sber/)" fires before the name is read only if the green is the right one. Shift the shade a little and recognition slows — and with it goes the whole point for which the brand claimed the color.

The second reason is graver than recognition: **legal protection**. A color can be part of a trademark, and then a "roughly this" shade is no longer an aesthetic but a legal problem. [Sber](../../logos/bank/sber/) registered its green, [T-Bank](../../logos/bank/tbank/) its yellow, precisely so a competitor couldn't mimic them by color. Using someone else's brand shade in your own mark in the same category is a direct risk; where the line of the permissible runs is covered in [can you use someone else's logo](../mozhno-li-ispolzovat-chuzhoy-logotip/).
...
There are essentially three sources, ordered by descending reliability:
...
:::danger How not to do it
An eyedropper on a logo from image search. There you'll find JPGs with compression artifacts, re-encoded copies and outright fakes. You'll grab #20A23C instead of #21A038 — seems close, yet next to the real one it looks dirty. An eyedropper is honest only as far as the file beneath it — and a file from search is almost always distorted.
:::

## How to use others' colors legally

A color's protected status doesn't forbid touching it at all. Showing a brand in its own color is fine: a "Log in with [VK](../../logos/social/vk/)" button in blue #0077FF, a list of banks with their brand icons, a review of services with their palettes. This is the same nominative (naming) use as with logos: you point at the brand while remaining yourself. What's forbidden is building **your own** identity on someone else's protected color in the same product category. A yellow bank, a green bank and a red carrier are already taken, and stepping onto that ground ends in a lawsuit.

Since we've mentioned that a color can be "taken," it's worth understanding how a color gets claimed legally at all.
...
There's one more reason a single HEX stopped covering every case — dark interfaces. A color tuned for a white background behaves differently on black: a saturated blue "vibrates," a dark green sinks, a yellow starts to shout. So mature design systems keep **two palette versions**: for light mode — the base codes from the brand book, for dark — adjusted ones, usually a touch lighter and less saturated.

For anyone using others' brand colors in their own interface, the takeaway is direct: check "Log in with…" buttons and bank icons in both themes. An official color on a non-native background may need a backing plate — which is exactly why payment-system guidelines describe in such detail which version of the badge to place on which background.

## Common questions

**Why didn't the color from the official site match the brand book?** Three reasons: a fresh redesign hasn't reached the site yet; you sampled the color with an eyedropper on a JPG (distortion); the designers deliberately use an "interface" version of the color, different from print. Trust order: fresh brand book → SVG logo → website.

**How to find a brand's color without public guidelines?** The SVG logo is the most reliable source: the color is written in the file's code. Download the vector and check `fill` — how exactly is shown in [how to find a logo's color](../kak-uznat-cvet-logotipa/).

**How many colors are in a brand palette?** The core is 1–2 brand colors, plus functional ones: 2–4 neutrals (text, backgrounds) and semantics (success/error/warning). Palettes of ten "brand" colors show up in ecosystems where each service needs its own accent.

## What to remember

A brand color is as much a brand asset as the logo, and it always has an exact code — because recognition, legal protection and consistency all rest on that precision. Hence the single rule: take the color from the primary source — a guideline or the SVG — don't pick it by eye and don't sample it from a search image.

In our [logo catalog](../../logos/) every brand has a "Brand colors" block: official HEX codes are extracted from the original SVG and copy in one click — faster than hunting down a brand book.
