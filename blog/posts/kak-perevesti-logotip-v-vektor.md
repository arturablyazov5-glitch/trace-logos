...
## Путь 1. Автотрассировка — 10 минут
...
## Путь 2. Ручная отрисовка — вечер работы
...
4. Цвета снимите пипеткой с крупных заливок исходника — а лучше найдите точные фирменные коды (как — в статье [про цвет логотипа](../kak-uznat-cvet-logotipa/)).
...
- **Прогнать через оптимизатор** — финальный файл станет в разы легче ([инструкция по SVGO](../kak-optimizirovat-svg/)).
...
4. Sample colors from large solid fills — or better, find the exact brand codes ([how](../kak-uznat-cvet-logotipa/)).
...
- **Assign exact colors.** Not "whatever the trace produced" but the brand HEX from guidelines or the source SVG ([how to find it](../kak-uznat-cvet-logotipa/)).
- **Run the optimizer** — the final file gets several times lighter ([the SVGO guide](../kak-optimizirovat-svg/)).
